...
const { IndexerSrv, NotificationSrv, DispatchSrv, BackupSrv } = require("./services");
...
mongoose.connect(config.mongoose.url, config.mongoose.options).then(async () => {
	...
	server = app.listen(config.port, async () => {
		...
		if (config.INDEXED_CHAINS.length) {
			logger.info(`INDEXED_CHAINS: ${config.INDEXED_CHAINS.map(c => c.id).join(' ')}`);
			
			if (config.INDEXED_CHAINS.length) {			
				for (const chain of config.INDEXED_CHAINS) {
					IndexerSrv.start(chain.id, parseInt(chain.delay) * 1000)	
				}								
