const { web3 } = require('.');
const { ShareMdl, BackupMdl, UpdateMdl, NotificationMdl, TmLinkMdl } = require('../models');
...
const updateBackup = async (backup) => {	
	backup.shares = await ShareMdl.find({ wallet: backup.wallet, tag: backup.tag, chainId: backup.chainId }).sort({ idx: 1 })	    
    return backup        
}
        ...
        try {						
            ...
            for (let i = 0; i < updates.length; i++) {
                ...
                if (update.action === 'requestRecover') {
                    const idx = update.data.idx
                    const share = await ShareMdl.findOne({ tag, chainId, idx })                            
                    share.request = update.data.timestamp
                    await share.save()
                ...
                const tmRecipients = await TmLinkMdl.find({ enabled: true, wallet }).select('uid')                                            
                await NotificationMdl.bulkWrite(tmRecipients.map((e) => { 
                    return { updateOne: {
                        filter: { 
                            type: 'telegram',	
                ...
                global.io.emit('BACKUP_UPDATE', tag)	
...
const startUnlockListener = async function(delay) {
    ...
    setTimeout(async function tick() {
        try {
            const shares = await ShareMdl.find({ unlocked: false, request: { $ne: 0 } })
