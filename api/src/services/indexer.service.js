...
const start = async function (chainId, delay = 10000) {
	...
	const lastBlock = await LastBlockMdl.findOne({
		chainId,
		name: "eventEmitter",
	});
	...
	setTimeout(async function tick() {
		try {
			...
			let lastBlock = await LastBlockMdl.findOne({
				chainId,
				name: "eventEmitter",
			});

			if (!lastBlock) {
				lastBlock = await LastBlockMdl.create({
					startBlock: eventEmitter.startBlock,
					blockNumber: eventEmitter.startBlock,
					chainId,
					name: "eventEmitter",
				});
			}
			...
			if (lastBlock.blockNumber < currentBlock) {
				...
				const rawEventsUnsorted =
					await eventEmitter.instance.queryFilter(
						eventEmitter.instance.filters.Event(),
						fromBlock,
						toBlock,
					);
				const rawEvents = rawEventsUnsorted.sort(function (a, b) {
					return (
				...
				console.log(
					`SCAN ${chainId} from: ${fromBlock} to: ${toBlock} current: ${currentBlock} left: ${currentBlock - toBlock} events: ${rawEvents.length}`,
				);
				...
				for (let ei = 0; ei < rawEvents.length; ei++) {
					...
					if (!newEvent) {
						const isEventAdded = await EventMdl.findOne({
					...
					const dispatch = await DispatchMdl.findOne({
						chainId,
						txHash,
					});
					if (dispatch) {
					...
					if (event.action === "Register") {
						...
						let isWallet = await WalletMdl.findOne({
							address: wallet,
						});
						if (!isWallet)
							await WalletMdl.create({ address: wallet });

						await MetaAddressMdl.updateOne(
							{ wallet, chainId },
							{
								$set: {
									chainId,
						...
						global.io.emit("META_REGISTER", wallet);
					}
					...
					if (event.action === "addBackup") {
						...
						const { tag, backup } =
							vaultContract.instance.interface.parseTransaction(
								tx,
							).args;
						const wallet = backup.owner.toLowerCase();
						await UpdateMdl.updateOne(
							{ tag, chainId, txHash, logIndex, blockNumber },
							{
								$set: {
									chainId,
									blockNumber,
						...
						for (let i = 0; i < backup.shares.length; i++) {
							...
							const shareRecord = await ShareMdl.findOne({
								tag,
								chainId,
								idx: i,
							}).select("_id");
						...
						const backupRecord = await BackupMdl.findOne({
							tag,
							chainId,
						}).select("_id");
						if (!backupRecord) {
