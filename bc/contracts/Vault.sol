...
contract Vault {
    ...
    bytes32 public constant ADD_BACKUP_TYPEHASH =
        keccak256(
    ...
    bytes32 public constant UPDATE_BACKUP_DISABLED_TYPEHASH =
        keccak256(
            "UpdateBackupDisabled(string tag,uint8 disabled,uint40 expire)"
        );
    bytes32 public constant UPDATE_SHARE_DISABLED_TYPEHASH =
        keccak256(
            "UpdateShareDisabled(string tag,uint8 idx,uint8 disabled,uint40 expire)"
        );
    bytes32 public constant UPDATE_SHARE_DELAY_TYPEHASH =
        keccak256(
            "UpdateShareDelay(string tag,uint8 idx,uint40 delay,uint40 expire)"
        );
    bytes32 public constant REQUEST_RECOVER_TYPEHASH =
        keccak256("RequestRecover(string tag,uint8 idx,uint40 expire)");
    ...
    function addBackup(
        ...
        require(
            portionsLength > 0 && portionsLength < type(uint8).max,
            "Bad shares length"
        );
        ...
        for (uint8 i = 0; i < portionsLength; i++) {
            for (uint8 j = i + 1; j < portionsLength; j++) {
                require(
                    backup.shares[i].stealthAddress !=
                        backup.shares[j].stealthAddress,
                    "Address duplicate"
                );
            }
        }
        ...
        for (uint256 i = 0; i < backup.shares.length; i++) {
            ...
            PORTIONS_ARRAY_HASH = bytes.concat(
                PORTIONS_ARRAY_HASH,
                keccak256(
                    abi.encode(
                        SHARE_TYPEHASH,
    ...
    function updateBackupDisabled(
        ...
        eventEmitter.emitEvent(
            ...
            abi.encode(tag, disabled)
        );
    }

    function updateShareDisabled(
        string calldata tag,
        uint8 idx,
        uint8 disabled,
        uint40 expire,
        ...
        if (share.disabled == 0 && share.request > 0) {
            share.request = uint40(block.timestamp);
        }

        eventEmitter.emitEvent(
            "updateShareDisabled",
            abi.encode(tag, idx, disabled)
        );
    }

    function updateShareDelay(
        string calldata tag,
        uint8 idx,
        uint40 delay,
        uint40 expire,
    ...
    function getBackups(
        string[] memory tags
    ) public view returns (Backup[] memory) {
