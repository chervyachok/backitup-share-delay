<template>
	<FullContentBlock v-if="$user.account">
		<template #header> New backup </template>
		<template #content>
				...
				<template v-if="$user.account.registeredMetaWallet">
					...
					<div class="row justify-content-center gx-2 mb-2 mt-3">
						<div class="col-lg-12 col-xl-10 mb-2">
							<button type="button" class="btn btn-dark w-100" @click="addPartiesFromContacts()">Select from contacts</button>
						</div>
					...
					<template v-if="wallets.length && !isInvalid">
						...
						<div class="row justify-content-center gx-2">
							<div class="col-lg-12 col-xl-10 mb-2">
								<button type="button" class="btn btn-dark w-100" @click="backup()" :disabled="tx?.status === 'PROCESSING'">Create</button>
							</div>
						</div>
					</template>
...
const secret = ref(location.origin.includes('loc_') ? '0x9f489378530d12beecbcf5a756183cab35f12aabe054a3ad1b99' : null);
const comment = ref(
	location.origin.includes('loc_')
		? 'My wallet key 0x9f489378530d12beecbcf5a756183cab35f12aabe054a3ad1b99 \n1-Bob 0x70997970C51812dc3A010C7d01b50e0d17dc79C8 \n2-Elis 0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC'
		: null,
);
const submitted = ref();
...
const checkWallets = async () => {
	for (let i = 0; i < wallets.value.length; i++) {
		...
		if (!wallet.metaPublicKey) {
			try {
				const metaPublicKey = await readContract($web3.wagmiAdapter.wagmiConfig, {
...
const applyPartiesFromContacts = async (addresses) => {
	wallets.value.push(...addresses.map((a) => ({ address: a, dirty: true })));
	checkWallets();
};
...
onMounted(async () => {
	if (!$user.account.registeredMetaWallet) {
	}
	generateTag();
	...
	try {
		...
		try {
			...
			if (d.account && d.account.publicKey === $user.account.publicKey) {
				const offchainBkp = {
					chats: $user.account.chats || [],
					rooms: $user.account.rooms || [],
